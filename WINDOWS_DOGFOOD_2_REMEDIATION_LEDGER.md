...
| DOG2-008 | critical | product/install/npm lifecycle policy | fixed; candidate-wide regression pending | The clean PowerShell install reached `cortextos install`, but npm 11.17 had skipped Claude Code's global postinstall, leaving broken shims with no native `claude.exe`; the same policy warned on local `node-pty`/`esbuild` scripts | Windows global installs explicitly trust only `@anthropic-ai/claude-code`, the root manifest trusts only `node-pty` and `esbuild`, focused install/manifest tests pass, and the failed live run stopped without false success; exact full gates and a zero-state rerun remain required |
