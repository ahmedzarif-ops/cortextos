...
| DOG2-004 | critical | product/security/Windows ACL | second fix; candidate-wide regression pending | Windows Server 2025 CI exposed that reading and mutating an image-provided descriptor can abort secret restriction when it contains stale provisioning SIDs; raw-SID enumeration avoided account-name translation but per-rule mutation still failed on the hosted image | ACL replacement now builds a fresh protected `FileSecurity` descriptor containing only current-user and LocalSystem access, avoiding every existing ACE; exact hosted Windows CI remains the proof gate |
