...
| DOG2-004 | critical | product/security/Windows ACL | diagnosis continuing; candidate-wide regression pending | Windows Server 2025 CI exposed that both mutating the image-provided descriptor and applying a fresh protected `FileSecurity` descriptor exit nonzero on the hosted image; secret writes correctly remain fail-closed | The adapter now retains a bounded, target-path-redacted PowerShell diagnostic so the next hosted run identifies the exact failing operation without exposing contents; no VM cleanup or onboarding may begin until the ACL contract passes |
