...
| WIN-019 | medium | product/diagnostics | fixed | Git/Node path casing and hosted-runner ACL ownership differ; ignoring global Git config also removes the runner's broad `safe.directory` exception, causing valid provenance to be discarded | Windows identity is case-insensitive; Git gets a closed helper PATH and an exact canonical per-command safe directory, never inherited PATH or wildcard/global config |
