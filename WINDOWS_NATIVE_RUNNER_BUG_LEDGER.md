...
| WIN-019 | medium | product/diagnostics | fixed | Git and Node report the same Windows repository root with different casing under some service identities, causing valid provenance to be discarded | Canonical Windows path identity is case-insensitive; a case-variant repository regression retains the real source commit |
