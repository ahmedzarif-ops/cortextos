...
Copy `references/ygs-routes.example.json` to a new evidence path and replace every `REPLACE` value. Capture separate byte-bound fleet, restore, spend, and trigger receipts. The validator requires the authoritative six-seat YGS roster, a fresh trigger receipt, per-seat and total weekly spend, per-seat MCP transcript and usage receipts, a canonical native-cron scan, and an explicit city-first/chief-last order. It reads Anthropic usage through the reviewed same-package `scripts/read-authenticated-usage.mjs` module, never through a caller-selected framework executable. That reader makes a fresh OAuth request without cache or state writes; the validator derives weekly remaining itself and compares it to the plan and receipt. The example itself is deliberately not executable until those receipts are supplied.
...
The validator performs no writes and prints no credentials. The usage reader accepts only an absolute `CTX_ROOT`, resolves the active account there or the process OAuth token, calls the fixed Anthropic endpoint directly, and constructs the authenticated/non-cached provenance fields itself. HTTP 401, a missing token, an unreadable response, missing or out-of-range utilization fields, or a stale validator timestamp all fail closed; unavailable measurement is never converted to zero. A substituted `${CTX_FRAMEWORK_ROOT}/dist/cli.js` is never executed. The validator hashes every referenced fleet/restore/spend/trigger/MCP artifact and rejects a plan or receipt value that differs from the independently measured weekly remaining; any source other than `cortextos-check-usage-api:anthropic-oauth`; any plan trigger whose metric, denominator, value, time, or source differs from the separate receipt bytes; any roster other than the authoritative six YGS seats; a past/non-next Sunday restore; null or moving models (including whitespace-padded aliases); invalid restore runtimes; any Hermes restore pin that differs from byte-verified live config; whitespace-padded model/provider/MCP-server pins or normalized MCP duplicates; missing or inconsistent spend; reused or cross-profile MCP evidence; a tool from the wrong required server; enabled native crons at the canonical profile path; incomplete ordering; or a snapshot-path/hash mismatch. `evidence_max_age_minutes` is mandatory and capped at 24 hours. Any error blocks the switch.
