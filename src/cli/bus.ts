...
busCommand
  ...
  .action((status: string, opts: { task?: string; timezone?: string; interval?: string }) => {
    ...
    // MERGE RESOLUTION (#15 identity guard x #10 org-resolved day/night mode): keep BOTH, and
    // pass the VERIFIED identity to the options builder too, not `env.agentName`. That builder
    // calls resolveModeSettings(frameworkRoot, org, agentName, ...) to read the agent's own
    // timezone and day-mode window. Signing with `me` while resolving mode from `env.agentName`
    // would read one agent's schedule onto another agent's liveness record whenever the two
    // differ — which is exactly the case #15 exists to make impossible.
    updateHeartbeat(paths, me, status, buildHeartbeatOptions(
      frameworkRoot, env.org, me, opts, displayName,
    ));
