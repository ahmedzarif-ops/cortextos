...
/**
 * Materialize the dashboard's gitignored Next.js env file from the credentials
 * created by `cortextos install`. Secrets stay out of ecosystem.config.js and
 * PM2 arguments. Existing .env.local files are preserved for compatibility.
 */
export function prepareDashboardEnvLocal(
  ctxRoot: string,
  dashboardDir: string,
  projectRoot: string,
  instance: string,
  ...
  if (existsSync(nextEnvPath)) return true;

  const credentialsPath = join(ctxRoot, 'dashboard.env');
  if (!existsSync(credentialsPath)) return false;
  const credentials: Record<string, string> = {};
  for (const line of readFileSync(credentialsPath, 'utf-8').split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith('#')) continue;
    const separator = trimmed.indexOf('=');
    if (separator > 0) credentials[trimmed.slice(0, separator)] = trimmed.slice(separator + 1);
  }
  if (!credentials.AUTH_SECRET || !credentials.ADMIN_PASSWORD) return false;

  writeSecretFileSync(nextEnvPath, [
    '# AUTO-GENERATED by cortextos ecosystem. To change credentials, edit:',
    `# ${credentialsPath}`,
    `AUTH_SECRET=${credentials.AUTH_SECRET}`,
    'AUTH_TRUST_HOST=true',
    `ADMIN_USERNAME=${credentials.ADMIN_USERNAME || 'admin'}`,
...
export const ecosystemCommand = new Command('ecosystem')
  ...
  .action(async (options: { instance: string; org?: string; output: string; dashboardHost?: string }) => {
    ...
    if (hasDashboard && !prepareDashboardEnvLocal(
      ctxRoot,
      dashboardDir,
      projectRoot,
      options.instance,
      ...
      console.warn('Dashboard credentials are not ready. Run `cortextos install` before starting the generated dashboard entry.');
    }
